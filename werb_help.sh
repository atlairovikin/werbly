...
fold -s -w '66' << 'EndOfHeredoc' | sed -e 's/^[^[:space:]]/      &/'
  ...
  werb [[help]|( [hail] <query> )|( [define|undefine] brew|<formula> )|( [init|deinit] )|( [install|uninstall] )|( [apply|unapply] <cask> )|( [man] [<num1> <num2>]|[] )]:
    ...
    undefine [brew]|<formula>:
      unsets input alias if it exists -- if argument is left null, then "brew" is assumed
    ...
    apply <cask>:
      sets a symlink of <cask> as an application -- by default, this uses the User's Applications folder, not the System's
    unapply <cask>:
